...
Quick reputation check for обычного TON-адреса (кошелёк/контракт) без транзакции. Возвращает `risk_level`, `signals`, снэпшот аккаунта и AI-пояснение — идеален для поля "Check Address" в TMA.
